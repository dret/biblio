...
# Andreas Wettstein, Institut f"ur Integrierte Systeme, ETH Z"urich, 11/1999
# email: wettstae@iis.ee.ethz.ch

# configuration: apart from the path /usr/bin/perl above you might want to 
# change:
...
-r  supresses  automatic inclusion of .aux generated by \\include-ed files
...
my $maxnames=999;   # max. number of autor names per entry
...
my %Lab2Bib;        # dito, but for occurence in bibliography
my %Aut2Pag;        # for each author gives array of pages cited/in bilio.
my %Aut2Bib;        # dito, but only occurence in bibliography
...
my $see="";     # string to seperate other and first author, or undef.
...
{ duplicate\$ empty\$           % field present?
    ...
    { duplicate\$ 'names :=     % memorize namelist in variable "names"
      num.names\$ duplicate\$ 
...
function{default.type}{author format numnames { $addedit } { $altedit } if\$ }
...
# The generated .bst file is kept if the user whishes so (-k option).
...
if(exists $opt{'i'}){
    ...
    $rep=($plain eq $prevplain) ? "\\airep$author" : "\\aitop$author"
      if $plain ne $author;
    print AIFILE "\\item[\\authorstyle{$rep}]";
...
# make a compressed, sorted list of pages
...
sub compressed_pages
{
    ...
    my %pages=%{$A};
    my %pagerep=%{$B};
    my ($prevpage,$prevrep,$res,$glue,$pagepending,$page)=("","","","","","");
    ...
    if($prevpage){
        ...
        $res.=($pagepending ? "$glue$prevrep" : "").", $pagerep";
        $pagepending=0;
        }
    }else{
        $res.="$pagerep";
    ...
    $res.="$glue$prevrep" if $pagepending;
    return $res;
}
